...
export async function deleteManyPosts(
  ...
  const API_BASE = process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000";
  const res = await fetch(`${API_BASE}/api/admin/post/delete-many`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      ...(token ? { Authorization: `Bearer ${token}` } : {}),
    },
    body: JSON.stringify({ ids: ids.map((v) => Number(v)) }),
  });
  if (!res.ok) {
    const text = await res.text();
    throw new Error(text || `Failed to delete posts: ${res.status}`);
  }
  return res.json();
}
